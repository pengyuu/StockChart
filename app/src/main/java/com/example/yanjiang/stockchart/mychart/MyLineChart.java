...
public class MyLineChart extends LineChart {
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            float deltaX = mXAxis != null ? mXAxis.mAxisRange : ((mData == null ? 0.f : mData.getDataSetCount()) - 1.f);
            if (xIndex <= deltaX && xIndex <= deltaX * mAnimator.getPhaseX()) {
                Entry e = mData.getEntryForHighlight(mIndicesToHighlight[i]);
                // make sure entry not null
                if (e == null || set.getEntryIndex(e) != mIndicesToHighlight[i].getDataIndex())
                    continue;
                ...
                float yValForXIndex1 = minuteHelper.getDatas().get(set.getEntryIndex(e)).cjprice;
                float yValForXIndex2 = minuteHelper.getDatas().get(set.getEntryIndex(e)).per;
                String time = minuteHelper.getDatas().get(set.getEntryIndex(e)).time;
